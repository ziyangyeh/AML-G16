...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-in", "--input", type=str, help="Input Directory", default="formatted_data"
    )
    parser.add_argument("-w", "--width", type=int, help="Windows Width", default=512)
    parser.add_argument(
        "-he", "--height", type=int, help="Windows height", default=None
    )
    ...
    Parallel(n_jobs=cpu_count())(
        delayed(generate_image_and_depth)(
            vtp_file, rmat_lst, width=args.width, height=args.height
        )
        for vtp_file in tqdm(vtp_lst, total=len(vtp_lst))
    )
