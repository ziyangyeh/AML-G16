...
def merge(obj, info, output_dir: str, fdi_map: dict):
    ...
    mesh.celldata["FDI"] = (
        fdi_f := generate_cell_labels_from_point_label(mesh, FDI=True)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-out", "--output", type=str, help="Output Directory", default="formatted_data"
    )
    ...
    assert (total_len := len(list(DATASET_DIR.glob("*/*/*.obj")))) == len(
    ...
    Parallel(n_jobs=cpu_count())(
        delayed(merge)(obj, info, args.output, fdi_map)
        for obj, info in tqdm(total_pairs, total=total_len)
    )
