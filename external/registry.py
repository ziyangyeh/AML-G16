...
class Registry:
    ...
    def register_module(self, name=None, force=False, module=None):
        ...
        if not (name is None or isinstance(name, str) or misc.is_seq_of(name, str)):
            raise TypeError("name must be either of None, an instance of str or a sequence" f"  of str, but got {type(name)}")
...
def build_from_cfg(cfg, registry, default_args=None):
    ...
    if "NAME" not in cfg:
        if default_args is None or "NAME" not in default_args:
            raise KeyError('`cfg` or `default_args` must contain the key "NAME", ' f"but got {cfg}\n{default_args}")
