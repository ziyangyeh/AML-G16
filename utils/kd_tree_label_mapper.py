...
def kd_label_mapper(src: vedo.Mesh, dst: o3d.geometry.TriangleMesh) -> vedo.Mesh:
    ...
    src_pcd_tree = o3d.geometry.KDTreeFlann(
        o3d.geometry.PointCloud(
            points=o3d.utility.Vector3dVector(np.asarray(src.points()))
        )
    )
    src_pcd_cc_tree = o3d.geometry.KDTreeFlann(
        o3d.geometry.PointCloud(
            points=o3d.utility.Vector3dVector(np.asarray(src.cell_centers()))
        )
    )

    dst_pcd = o3d.geometry.PointCloud(points=dst.vertices)
    dst_cc_pcd = o3d.geometry.PointCloud(
        points=o3d.utility.Vector3dVector(
            np.asarray(vedo.utils.open3d2vedo(dst).cell_centers())
        )
    )

    pts_idx = np.apply_along_axis(
        get_index_from_o3d_kdtree, 1, np.asarray(dst_pcd.points), src_pcd_tree
    )
    face_idx = np.apply_along_axis(
        get_index_from_o3d_kdtree, 1, np.asarray(dst_cc_pcd.points), src_pcd_cc_tree
    )
